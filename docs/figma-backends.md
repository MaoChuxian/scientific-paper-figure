...
TalkToFigma completed a 44-step native/scientific run in the same dedicated file on channel `m4omllkl`: document/selection/design reads, six stage frames and labels, connector fixtures, Auto Layout direction/padding/spacing, fill/stroke, hug sizing, manual label/stage edits, overlap and clipping correction, and deletion read-back. A separate preserve-set fixture on the restarted channel `2qf7edgv` compared the untouched panel before and after a targeted edit and returned `preserveSetStable: true`. A focused large-root export retest with a 120-second outer MCP timeout failed again after about 30 seconds with `Error exporting node as image: Request to Figma timed out`; the earlier small TalkToFigma smoke export passed. This is recorded as an internal export-path timeout, not an outer client timeout. All retest frames were explicitly deleted afterward.
