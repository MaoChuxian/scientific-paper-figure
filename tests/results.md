...
- Figwright render recovery: after a working Scenario C root render, `save_screenshots` and a regional render stalled at the 120-second backend timeout. The local Figwright leader was stopped while the Figma plugin remained open; a follower took over the listening service. The old MCP transport correctly returned `Transport closed`, while a fresh MCP client restored end-to-end `ping` in 42 ms and rendered the 1490 x 240 Step 1 region in 7.4 seconds. This verifies backend restart plus MCP rebind as a recovery sequence; backend restart alone is not QA evidence.
